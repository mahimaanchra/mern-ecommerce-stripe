...
const PlaceOrder = () => {
  ...
  const {navigate , backendUrl , token , cartItems , setCarItems , getCartAmount , delivery_fee , product } = useContext(ShopContext);
    ...
    try {
      ...
      switch(method){
        ...
        case 'cod':
          ...
          if(responseRazorpay.data.succes){
...
<input onChange={onChangeHandler} name='street' value={formData.street}  className='border border-gray-300 rounded py-1.5 px-3.5 w-full' type='email' placeholder='Street'/>
<div className='flex gap-3'>
